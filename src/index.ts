...
io.on('connection', (socket: Socket) => {
  ...
  socket.on('joinRoom', (roomCode: string, participantName: string) => {
    const roomToEnter = currentRooms.filter((roomObject) => roomObject.code === roomCode);
  ...
  socket.on('addNote', (roomCode: string, noteToAdd: Note) => {
    const roomToEnter = currentRooms.filter((roomObject) => roomObject.code === roomCode)[0];

    const existingNote = roomToEnter.notes.filter((note) => note.id === noteToAdd.id && note.author === noteToAdd.author)[0];
